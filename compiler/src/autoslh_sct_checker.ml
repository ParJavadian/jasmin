...
let pp_gvar fmt x = pp_var_i fmt x.gv
...
type sct_error = { err : hierror; info : info option }

exception SCTError of sct_error

let error ~loc ?funname ?sub_kind ?(internal = false) ?info =
  let mk pp =
    let err =
      {
        err_msg = pp;
...
let pp_opt pp =
  let pp_none fmt () = Format.fprintf fmt "None" in
  let pp_some fmt x = Format.fprintf fmt "Some %a" pp x in
  Format.pp_print_option ~none:pp_none pp_some

let pp_info fmt i =
  Format.fprintf fmt "@[{ loc: %d; var: %a; }@]" i.loc pp_gvar i.var

let pp_sct_error fmt err =
  Format.fprintf fmt "@[err = %a@]\n@[info = %a@]@." pp_hierror err.err
    (pp_opt pp_info) err.info
...
type vty = Direct of VlPairs.t | Indirect of VlPairs.t * VlPairs.t

let pp_vlpair ~top fmt (n, s) =
  if Lvl.is_public n && Lvl.is_public s then
    Format.fprintf fmt "%s%s" (if top then "#" else "") spublic
  else if Lvl.is_public n && Lvl.is_secret s then
    Format.fprintf fmt "%s%s" (if top then "#" else "") stransient
  else if Lvl.is_secret n && Lvl.is_secret s then
    Format.fprintf fmt "%s%s" (if top then "#" else "") ssecret
  else
    Format.fprintf fmt "%s{ n = %a, s = %a}"
      (if top then "#poly = " else "")
      Lvl.pp n Lvl.pp s

let pp_vty fmt = function
  | Direct vlp -> pp_vlpair ~top:true fmt vlp
  | Indirect (p_vlp, v_vlp) ->
      Format.fprintf fmt "#[ptr = %a, val = %a]" (pp_vlpair ~top:false) p_vlp
        (pp_vlpair ~top:false) v_vlp

type vfty = IsMsf | IsNormal of vty
...
type modmsf = Modified of L.i_loc * (L.i_loc * funname) list | NotModified

let is_Modified m = match m with Modified _ -> true | NotModified -> false
...
type ty_fun = {
  ...
  resulting_corruption : VlPairs.t;
      (* resulting memory corruption after function call *)
}
...
module FEnv = struct
  let get_fun_def fenv fn =
    List.find (fun f -> F.equal f.f_name fn) fenv.env_def
...
let pp_modmsf fmt modmsf =
  let s =
    match modmsf with Modified _ -> "modmsf" | NotModified -> "nomodmsf"
  in
  Format.fprintf fmt "%s" s

let pp_funty fmt (fname, tyfun) =
  Format.fprintf fmt
    "@[<v>%a %s : @[%a@] ->@ @[%a@]@ output corruption: %a@.@ constraints:@ \
     @[%a@]@]@."
    pp_modmsf tyfun.modmsf fname (pp_list " *@ " pp_vfty) tyfun.tyin
    (pp_list " *@ " pp_vfty) tyfun.tyout pp_vty
    (Direct tyfun.resulting_corruption) C.pp tyfun.constraints
...
and modmsf_c fenv c =
  ...
  |> List.find_opt is_Modified |> Option.default NotModified
...
let ensure_register ~direct x =
  if not (is_register ~direct (L.unloc x)) then
    error ~loc:(L.loc x) "variable %a must be a reg%s" pp_var_i x
      (if direct then "" else " (ptr)")

let reg_lval ~direct loc x =
  match x with
  | Laset (_, _, _, x, _) | Lvar x ->
      ensure_register ~direct x;
      x
  | _ ->
      error ~loc "L-value %a must be a reg%s" pp_lval x
        (if direct then "" else " (ptr)")

let reg_lval_opt ~direct loc = function
  | Lnone _ -> None
  | x -> Some (reg_lval ~direct loc x)

let reg_expr_opt ~direct = function
  | Pget (_, _, _, x, _) | Pvar x ->
      if is_gkvar x && is_register ~direct (L.unloc x.gv) then Some x.gv
      else None
  | _ -> None
...
let reg_expr ~direct loc e =
  ...
  | Some x -> x
  | None ->
      error ~loc "expression %a must be a reg%s" pp_expr e
        (if direct then "" else " (ptr)")
...
let rec infer_msf_i ~withcheck fenv (tbl : (L.i_loc, Sv.t) Hashtbl.t) i ms =
  ...
  let check_x ms x =
    if Sv.mem (L.unloc x) ms then
      error ~loc:(L.loc x)
        "instruction assigns %a, which is required to be a msf" pp_var_i x
  in

  let check ms x = match x with Lvar x -> check_x ms x | _ -> () in

  let checks ms xs = List.iter (check ms) xs in

  let pp_modmsf_trace fmt tr =
    let pp_item fmt (l, fn) =
      Format.fprintf fmt "@[<h>the function %s destroys MSFs at %a@]" fn.fn_name
        L.pp_iloc l
    in
    ...
    | Modified (l, tr) ->
        if (not (Sv.is_empty ms)) && withcheck then
          error ~loc
            ...
            pp_vset ms pp_modmsf_trace ((l, fn) :: tr)
    | NotModified -> ()
  in
  ...
  | Cwhile (_, c1, _, c2) ->
      ...
      let rec loop ms =
        ...
        if Sv.subset ms2 ms then (
          Hashtbl.add tbl i.i_loc ms1;
          ms1)
        else loop (Sv.union ms2 ms)
      in
      loop ms
  | Cassgn (Lvar x, tag, _, e) when Sv.mem (L.unloc x) ms -> (
      ...
      in
      match reg_expr_opt ~direct:true e with
  ...
  | Ccall (xs, f, es) ->
      ...
      let doin ms vfty e =
        match vfty with
        | IsMsf ->
            let x = reg_expr ~direct:true loc e in
            Sv.add (L.unloc x) ms
        | _ -> ms
      in
  ...
  | Copn (xs, _, o, es) -> (
      ...
      | Init_msf, [ x ], _ ->
          let x =
            Option.map_default
              (fun x -> Sv.singleton (L.unloc x))
              Sv.empty
              (reg_lval_opt ~direct:true loc x)
          in
          if not (Sv.subset ms x) then
            error ~loc
              "only %a is ensured to be msf after init_msf, %a are required"
              pp_vset x pp_vset ms;
          Sv.empty
      | Init_msf, _, _ -> assert false
      | Update_msf, [ xms ], [ _e; ms' ] ->
          let xms = reg_lval ~direct:true loc xms
          and ms' = reg_expr ~direct:true loc ms' in
          if not (Sv.subset ms (Sv.singleton (L.unloc xms))) then
            error ~loc
              "only %a is ensured to be msf after update_msf, %a are required"
              pp_var_i xms pp_vset ms;
          Sv.singleton (L.unloc ms')
      | Update_msf, _, _ -> assert false
      | Mov_msf, [ x ], [ x' ] ->
          let x = reg_lval ~direct:true loc x
          and x' = reg_expr ~direct:true loc x' in
          Sv.add (L.unloc x') (Sv.remove (L.unloc x) ms)
...
let pp_kind fmt k =
  Format.fprintf fmt "%s" (if k = Strict then sstrict else sflexible)

let error_unsat ?info loc (_ : Lvl.t list * Lvl.t * Lvl.t) pp e ety ety' =
  error ~loc "%a has type %a but should be at most %a" pp e pp_vty ety pp_vty
    ety' ?info
...
module Env : sig
  ...
  val dpublic : env -> vty
  val dsecret : env -> vty
  val get : public:bool -> venv -> ?iloc:L.i_loc -> var -> vty
  val get_i : public:bool -> venv -> ?iloc:L.i_loc -> var_i -> vty
  val gget : public:bool -> venv -> ?iloc:L.i_loc -> int ggvar -> vty
  val kind : env -> var_i -> var_kind
  ...
  val ensure_le : L.i_loc -> venv -> venv -> unit
  ...
  type env = {
    ...
    spilled : var option Hv.t;
        (* None means that the variable is spill to mmx and not in the stack *)
  ...
  let fresh2 ?name env =
    (C.fresh ?name env.constraints, C.fresh ?name env.constraints)
  ...
  let get ~(public : bool) venv ?iloc x : vty =
    match x.v_kind with
    | Global -> Direct venv.public2
    | _ ->
        let lvl = try Mv.find x venv.vtype with Not_found -> assert false in
        if public then
          match lvl with
          | Direct ty | Indirect (ty, _) -> (
              try
                let _ = VlPairs.add_le ty venv.public2 in
                lvl
              with Lvl.Unsat _ ->
                let loc =
                  match iloc with
                  | Some iloc -> iloc.L.base_loc
                  | None -> L._dummy
                in
                let uid =
                  match iloc with Some iloc -> iloc.L.uid_loc | None -> 0
                in
                let x_ggvar = gkvar (Location.mk_loc L._dummy x) in
                let info = { loc = uid; var = x_ggvar } in
                error ~loc "%a has type %a but should be at most #%s" pp_gvar
                  x_ggvar pp_vty lvl spublic ~info)
        else lvl

  let get_i ~(public : bool) venv ?iloc x = get ~public venv ?iloc (L.unloc x)
  let gget ~(public : bool) venv ?iloc x = get_i ~public venv ?iloc x.gv

  let kind env x =
    if Hv.mem env.strictness (L.unloc x) then Strict else Flexible

  let add_le_var ty1 ty2 =
    match (ty1, ty2) with
    | Direct ty1, Direct ty2 -> VlPairs.add_le ty1 ty2
    | Indirect (l1, x1), Indirect (l2, x2) ->
        VlPairs.add_le l1 l2;
        VlPairs.add_le x1 x2
    | _ -> assert false
  ...
  let init_ty env venv x xty =
    ...
    let nxty =
      ...
      | _, _ -> assert false
    in
    { venv with vtype = Mv.add x nxty venv.vtype; vars = Sv.add x venv.vars }
  ...
  let set_ty env venv x nxty =
    let xty = get_i ~public:false venv x in
    try
      if kind env x = Flexible then init_ty env venv (L.unloc x) nxty
      else (
        add_le_var xty nxty;
        venv)
    with Lvl.Unsat _unsat ->
      error ~loc:(L.loc x)
        "%a has strict type %a, it cannot receive a value of type %a" pp_var_i x
        pp_vty xty pp_vty nxty

  (* Initialises type for InitMsf operations. Acts as a Fence operator *)
  let set_init_msf env venv ms =
    let venv =
      match ms with Some ms -> set_ty env venv ms (dpublic env) | None -> venv
    in
    let operate_fence _ = function
      | Direct le -> Direct (VlPairs.normalise le)
      | Indirect (lp, le) ->
          Indirect (VlPairs.normalise lp, VlPairs.normalise le)
    in
  ...
  let max env venv1 venv2 =
    ...
    let merge (n1, s1) (n2, s2) = (merge1 n1 n2, merge1 s1 s2) in
    ...
    let merge_var _ oty1 oty2 =
      ...
      | Indirect (lp1, le1), Indirect (lp2, le2) ->
          Some (Indirect (merge lp1 lp2, merge le1 le2))
      | _ -> assert false
    in
    {
      venv1 with
      ...
      resulting_corruption =
        merge venv1.resulting_corruption venv2.resulting_corruption;
    }
        ...
        Sv.fold
          (fun x vtype ->
            let in_memory =
              match x.v_kind with
              | Wsize.Global
                (* likely unused as global variables are not in venv.vars *)
              | Stack _ ->
                  true
            ...
            in
            Mv.add x ty vtype)
          venv.vars venv.vtype;
    }

  let ensure_le iloc venv1 venv2 =
    let add_le_silent x oty1 oty2 =
      try
        add_le_var (oget oty1) (oget oty2);
        None
      with Lvl.Unsat _unsat ->
        let gx = gkvar (Location.mk_loc L._dummy x) in
        let info = { loc = iloc.L.uid_loc; var = gx } in
        error ~loc:iloc.L.base_loc ~info
          "constraints caused by the loop cannot be satisfied"
    in
    ignore (Mv.merge add_le_silent venv1.vtype venv2.vtype)
    ...
    let subst_ty = function
      ...
      | IsNormal ty ->
          let ty =
            ...
            | Indirect (lp, le) -> Indirect (subst lp, subst le)
          in
          IsNormal ty
    in
    ( List.map subst_ty tyfun.tyout,
      List.map subst_ty tyfun.tyin,
  ...
  let corruption_speculative env venv (_, s) =
    corruption env venv (public env, s)
    ...
    let sx =
      if CoreIdent.Cident.spill_to_mmx x then None
        ...
        let kind =
          ...
          | Global ->
              if is_ty_arr x.v_ty then Wsize.Stack (Pointer Constant)
              else Wsize.Stack Direct
          | Reg (_, r) -> Stack r
        in
  ...
  let set_spill env venv xs =
    let add venv (x : var_i) =
      Option.map_default
        (fun sx ->
          let ty = get_i ~public:false venv x in
          set_ty env venv sx ty)
        venv (get_spilled env x)
    in
  ...
  let set_unspill env venv xs =
    let add venv (x : var_i) =
      Option.map_default
        (fun sx ->
          let ty = get_i ~public:false venv sx in
          set_ty env venv x ty)
        venv (get_spilled env x)
    in
    List.fold_left add venv xs
end
...
let content_ty = function Direct le | Indirect (_, le) -> le

(* --------------------------------------------------------- *)
(* Type checking of expressions                              *)

let rec ty_expr ~(public : bool) env venv loc (iloc : L.i_loc) (e : expr) : vty
    =
  match e with
  | Pconst _ | Pbool _ | Parr_init _ -> Env.dpublic env
  | Pvar x -> Env.gget ~public venv ~iloc x
  | Pget (_, aa, ws, x, i) ->
      ensure_public_address env venv loc iloc x.gv;
      ensure_public env venv loc iloc i;
      let ty = Env.fresh2 env and xty = Env.gget ~public venv ~iloc x in
      if not (ssafe_test x.gv aa ws i) then
  ...
  | Psub (_, _, _, x, i) ->
      ensure_public env venv loc iloc i;
      Env.gget ~public venv ~iloc x
  | Pload (_, _, x, i) ->
      ensure_public env venv loc iloc (Pvar (gkvar x));
      ensure_public env venv loc iloc i;
      Env.dsecret env
  | Papp1 (o, e) ->
      let public = (not (CT.is_ct_op1 o)) || public in
      ty_exprs_max ~public env venv loc iloc [ e ]
  | Papp2 (o, e1, e2) ->
      let public = (not (CT.is_ct_op2 o)) || public in
      ty_exprs_max ~public env venv loc iloc [ e1; e2 ]
  | PappN (o, es) ->
      let public = (not (CT.is_ct_opN o)) || public in
      ty_exprs_max ~public env venv loc iloc es
  | Pif (_, e1, e2, e3) -> (
      let ty1 = ty_expr ~public env venv loc iloc e1 in
      let ty2 = ty_expr ~public env venv loc iloc e2 in
      let ty3 = ty_expr ~public env venv loc iloc e3 in
      match ty1 with
      ...
      | Direct l1 -> (
          let do_indirect lp2 le2 lp3 le3 =
            ...
            VlPairs.add_le l1 lp;
            VlPairs.add_le lp2 lp;
            VlPairs.add_le lp3 lp;
            VlPairs.add_le l1 le;
            VlPairs.add_le le2 le;
            VlPairs.add_le le3 le;
            Indirect (lp, le)
          in
          match (ty2, ty3) with
          | Direct l2, Direct l3 ->
              let le = Env.fresh2 env in
              VlPairs.add_le l1 le;
              VlPairs.add_le l2 le;
              VlPairs.add_le l3 le;
              Direct le
...
and ensure_smaller env venv loc (iloc : L.i_loc) e (l : VlPairs.t) : unit =
  let ety = ty_expr ~public:false env venv loc iloc e in
  match ety with
  | Direct le | Indirect (le, _) -> (
      try VlPairs.add_le le l
      with Lvl.Unsat unsat ->
        ignore (ty_expr ~public:true env venv loc iloc e))

and ensure_public env venv loc (iloc : L.i_loc) e : unit =
  ignore (ty_expr ~public:true env venv loc iloc e)

and ensure_public_address env venv loc (iloc : L.i_loc) x : unit =
  let ety = Env.get_i ~public:false venv x in
  match ety with
  | Direct _ -> () (* stack or reg arrays have public addresses by definition *)
  | Indirect (le, _) -> (
      try VlPairs.add_le le (Env.public2 env)
      with Lvl.Unsat unsat -> ignore (Env.get_i ~public:true venv x ~iloc))

and ty_exprs_max ~(public : bool) env venv loc (iloc : L.i_loc) es : vty =
  let l = if public then Env.public2 env else Env.fresh2 env in
  List.iter (fun e -> ensure_smaller env venv loc iloc e l) es;
  Direct l
...
let expr_equal a b =
  ...
  in
  let normalize e =
    e |> Conv.cexpr_of_expr |> Constant_prop.(const_prop_e fcp None empty_cpm)
  in
...
module MSF : sig
    ...
    Stdlib.Option.iter
      (fun e ->
        if Sv.mem x (vars_e e) then
          error ~loc
            "%a cannot become an MSF as the current status depends on it (%a)"
            pp_var x pp_expr e)
      oe;
    (Sv.add x xs, oe)
  ...
  let enter_if msf e =
    match msf with _, Some _ -> toinit | xs, None -> (xs, Some e)
  ...
  let check_msf (xs, _) ms =
    if not (Sv.mem (L.unloc ms) xs) then
      error ~loc:(L.loc ms)
        "the variable %a is not known to be a msf, only %a are" pp_var_i ms
        pp_vset xs
  ...
  let check_msf_trans ((_, ob) as msf) ms b =
    ...
    | Some b' ->
        if not (expr_equal b b') then
          error ~loc:(L.loc ms) "the expression %a need to be equal to@ %a"
            pp_expr b pp_expr b';
        check_msf msf ms
  ...
  let is_msf_exact (xs, ob) ms =
    match ob with Some _ -> false | None -> Sv.mem (L.unloc ms) xs
  ...
  let loop env loc ((xs, oe) as msf) =
    ...
    if Sv.subset xs' xs then (xs', oe)
      ...
      error ~loc:loc.L.base_loc "current msf = %a, it should contain %a" pp msf
        pp_vset (Sv.diff xs' xs)
    ...
    if Sv.subset xsi xso then (
      ...
      | _, _ ->
          if not (Sv.is_empty xsi) then
            error ~loc "msf is %a it should be be at least %a" pp msfo pp msfi;
          toinit)
    else error ~loc "msf is %a it should be be at least %a" pp msfo pp msfi
end
...
let ty_lval env (iloc : L.i_loc) ((msf, venv) as msf_e : msf_e) x ety : msf_e =
  ...
  | Lvar x ->
      ...
      let lp, le =
        ...
        | Indirect (lp, le) -> (lp, le)
      in
      let xty = if is_ptr (kind_i x) then Indirect (lp, le) else Direct le in
      let msf = MSF.update msf (L.unloc x) in
      let venv = Env.set_ty env venv x xty in
      (msf, venv)
  | Lmem (_, _, x, i) ->
      ensure_public env venv (L.loc x) iloc (Pvar (gkvar x));
      ensure_public env venv (L.loc x) iloc i;
  ...
  | Laset (_, aa, ws, x, i) ->
      ensure_public_address env venv (L.loc x) iloc x;
      ensure_public env venv (L.loc x) iloc i;
      let le = content_ty ety in
      let venv =
        let l = Env.fresh2 env in
        let xty =
          match Env.get_i ~public:false venv x with
          | Direct lx ->
              VlPairs.add_le lx l;
              VlPairs.add_le le l;
              Direct l
          | Indirect (lp, lx) ->
              ...
              Indirect (lp, l)
        in
        Env.set_ty env venv x xty
      in

      if ssafe_test x aa ws i then (msf, venv)
      else
        (* mispeculation has necessarily occured *)
...
let ty_lvals1 env (iloc : L.i_loc) (msf_e : msf_e) xs ety : msf_e =
  List.fold_left (fun msf_e x -> ty_lval env iloc msf_e x ety) msf_e xs

let ty_lvals env (iloc : L.i_loc) (msf_e : msf_e) xs tys : msf_e =
  List.fold_left2 (ty_lval env iloc) msf_e xs tys
...
let declassify_ty env annot ty =
  if is_declassify annot then declassify env ty else ty

let declassify_tys env annot tys =
  if is_declassify annot then List.map (declassify env) tys else tys

(* right now only used by syscall, which only consists of randombytes
   it is thus tailored for this specific function. *)
let ensure_public_address_expr env venv loc (iloc : L.i_loc) e =
  let ety = ty_expr ~public:false env venv loc iloc e in
  match ety with
  | Direct _ -> ()
  | Indirect (le, _) -> (
...
(* Inline variables must be public, as required in [init_ty]. *)
(* TODO: Reg bool variables *that leak* should also return true. *)
(* TODO: Strict public variables should also return true. *)
let needs_public_rhs = function
  | Lvar x -> (L.unloc x).v_kind = Inline
  | _ -> false

(* [ty_instr env msf i] return msf' such that env, msf |- i : msf' *)
let rec ty_instr is_ct_asm fenv env ((msf, venv) as msf_e : msf_e) i =
  let iloc = i.i_loc in
  let loc = iloc.L.base_loc in
  match i.i_desc with
  | Csyscall (xs, o, es) ->
      (* TODO: generalize to other syscalls *)
      assert (match o with Syscall_t.RandomBytes _ -> true);
      List.iter (ensure_public_address_expr env venv loc iloc) es;
      (* We don't known what happen to MSF after external function call *)
      ty_lvals1 env iloc (MSF.toinit, venv) xs (Env.dsecret env)
  | Cassgn (mso, _, _, (Pvar x as msi)) when MSF.is_msf msf x.gv ->
      move_msf ~loc env msf_e mso msi
  | Cassgn (x, _, _, e) ->
      let ety = ty_expr ~public:(needs_public_rhs x) env venv loc iloc e in
      ty_lval env iloc msf_e x (declassify_ty env i.i_annot ety)
  | Copn (xs, _, o, es) -> (
      match (is_special o, xs, es) with
      | Init_msf, [ ms ], _ ->
      ...
      | Update_msf, [ mso ], [ b; msi ] ->
          ...
          let _, venv = ty_lvals1 env iloc (msf, venv) xs (Env.dpublic env) in
          (MSF.exact1 mso, venv)
      | Update_msf, _, _ -> assert false
      | Mov_msf, [ mso ], [ msi ] -> move_msf ~loc env msf_e mso msi
      | Mov_msf, _, _ -> assert false
      | Protect, [ x ], [ e; ms ] ->
          let _ = reg_lval ~direct:false loc x
          and _ = reg_expr ~direct:false loc e
          and ms = reg_expr ~direct:true loc ms in
          MSF.check_msf_exact msf ms;
          let xty =
            match ty_expr ~public:false env venv loc iloc e with
            | Direct (n, _) -> Direct (n, n)
            | Indirect ((n, _), le) -> Indirect ((n, n), le)
          in

          ty_lval env iloc msf_e x xty
      | Protect, _, _ -> assert false
      | Spill o, _, es ->
      ...
      | Other, _, _ ->
          let public =
            List.exists needs_public_rhs xs || not (CT.is_ct_sopn is_ct_asm o)
          in
          let ety = ty_exprs_max ~public env venv loc iloc es in
          ty_lvals1 env iloc msf_e xs (declassify_ty env i.i_annot ety))
  | Cif (e, c1, c2) ->
      if is_inline i then
        ...
        ensure_public env venv loc iloc e;
        let msf1, venv1 =
          ty_cmd is_ct_asm fenv env (MSF.enter_if msf e, venv) c1
        in
        let msf2, venv2 =
          ty_cmd is_ct_asm fenv env
            (MSF.enter_if msf (Papp1 (Onot, e)), venv)
            c2
        in
        (MSF.max msf1 msf2, Env.max env venv1 venv2))
  | Cfor (x, (_, e1, e2), c) ->
      ensure_public env venv loc iloc e1;
      ensure_public env venv loc iloc e2;

      let msf = MSF.loop env i.i_loc msf in
      (* let w, _ = written_vars [i] in *)
      let venv1 = Env.freshen env venv in
      (* venv <= venv1 *)
      let msf_e = ty_lval env iloc (msf, venv1) (Lvar x) (Env.dpublic env) in
      let msf', venv' = ty_cmd is_ct_asm fenv env msf_e c in
      let msf' = MSF.end_loop loc msf msf' in
      Env.ensure_le iloc venv' venv1;
      (* venv' <= venv1 *)
      (msf', venv1)
  | Cwhile (_, c1, e, c2) ->
      (* c1; while e do (c2; c1) *)
      ...
      *)
      ...
      ensure_public env venv2 loc iloc e;
      let msf', venv' =
        ty_cmd is_ct_asm fenv env (MSF.enter_if msf2 e, venv2) c2
      in
  ...
  | Ccall (xs, f, es) ->
        ...
        | IsNormal ety' -> (
            ...
            try
              ...
              | Indirect (lp, le), Direct le' ->
                  VlPairs.add_le lp (Env.public2 env);
                  VlPairs.add_le le le'
              | Indirect (lp, le), Indirect (lp', le') ->
                  VlPairs.add_le lp lp';
                  VlPairs.add_le le le'
            with Lvl.Unsat unsat ->
              ignore (ty_expr ~public:true env venv loc iloc e))
      in
      ...
      let output_ty msf_e x vfty =
        let ty =
          ...
          | IsNormal ty -> declassify_ty env i.i_annot ty
        in
        let msf, venv = ty_lval env iloc msf_e x ty in
        let msf =
          if vfty = IsMsf then MSF.add (reg_lval ~direct:true loc x) msf
          else msf
        in
        (msf, venv)
      in
...
let parse_var_annot ~(kind_allowed : bool) ~(msf : bool) (annot : annotations) :
    ulevel list * var_kind =
  let module A = Annot in
  let kind =
    let check_allowed (id, _) =
      if not kind_allowed then
        A.error ~loc:(L.loc id) "%s not allowed here" (L.unloc id)
    in
    let filters =
      [
        ( sflexible,
...
let parse_constraints (s : string) : (string * string) list =
  let error expected s n = raise (Error_after (expected, String.sub s 0 n)) in

  let is_blank c = c = ' ' || c = '\n' || c = '\t' in

  let rec check_blank expected s first last =
    if first >= last then ()
    else if is_blank s.[first] then check_blank expected s (first + 1) last
    else error expected s first
  in

  let search expected re s n =
    let first =
      try Str.search_forward re s n with Not_found -> error expected s n
    in
  ...
  let rec skip_blank s n =
    if n < String.length s && is_blank s.[n] then skip_blank s (n + 1) else n
  in
...
let parse_user_constraints (a : annotations) : (string * string) list =
  ...
  let on_string loc _ s =
    ...
    with Error_after (kw, s) -> A.error ~loc "%s expected after %s" kw s
  in
  let error loc =
    A.error ~loc "attribute for %s should be a string" sconstraints
  in
...
let init_constraint fenv f =
  let sig_annot =
    SecurityAnnotations.get_sct_signature f.f_annot.f_user_annot
  in
  let env = Env.init () in
  ...
  let to_lvl = function
    ...
    | Public | Msf -> Env.public2 env
  in

  let lvl_of_sa_level =
    let open SecurityAnnotations in
    let lvl_of_simple_level get = function
  ...
  let to_vty = function
    ...
    | Indirect { ptr; value } ->
        Indirect (lvl_of_sa_level ptr, lvl_of_sa_level value)
  in

  let error_msf loc = error ~loc "%s annotation not allowed here" smsf in
  ...
  let mk_vty loc ~is_local ~(msf : bool) x ls an =
    let msf, ovty =
      ...
      | _, None ->
          error ~loc:x.v_dloc "invalid security annotations %a" pp_var x
      | [ Msf ], Some n ->
          if not msf then error_msf loc;
          (Some true, Some (to_vty n))
      | _, Some n -> (Some (n = SecurityAnnotations.Msf), Some (to_vty n))
    in
    let vty =
      match ovty with
      | None -> (
          ...
          | Stack (Pointer _) when is_local ->
              Indirect ((Env.fresh env, Env.secret env), Env.fresh2 env)
      ...
      | Some ty ->
          (* this partly has the same role as Env.init_ty. Remove one occurence? *)
          (match (x.v_kind, ty) with
          | Const, Direct _ -> ()
          | Stack Direct, Direct _ -> ()
          | Stack (Pointer _), Indirect _ -> ()
          | Reg (_, Direct), Direct _ -> ()
          ...
          | Inline, Direct _ -> ()
          | Global, Direct _ -> ()
          | _ -> error ~loc "invalid security annotations for %a" pp_var x);
          ty
    in
    (msf, vty)
  in
  ...
  let process_return i x annot =
    ...
    mk_vty ~is_local:false loc ~msf:(not export) x ls an
  in
  ...
  (if export && not (Sv.is_empty msfs) then
     let vars_kind, pos =
       if Sv.subset msfs (Sv.of_list f.f_args) then
         ("arguments", ", this is not allowed for export functions")
       else ("variables", "")
     in
     error ~loc:f.f_loc "@[<h>the %s %a need to be MSFs%s.@]" vars_kind pp_vset
       msfs pos);
  ...
  let process_param i venv x =
    ...
    let ls, vk =
      parse_var_annot ~kind_allowed:true ~msf:(not export) x.v_annot
    in
    ...
    let msf =
      ...
      | Some b ->
          (if b <> Sv.mem x msfs then
             let loc = x.v_dloc in
             if b then warn ~loc "%a does not need to be an MSF" pp_var x
             else error ~loc "%a should be an MSF" pp_var x);
          b
    in
    (if export then
       let lvls =
         match vty with Indirect (p, v) -> [ p; v ] | Direct v -> [ v ]
       in
       List.iter
         (fun l ->
           try VlPairs.add_le (Env.public env, Env.secret env) l
           with Lvl.Unsat _unsat ->
             (* No need to provide info because we have an MSF initialization. *)
             error ~loc:x.v_dloc
               "security annotation for %a should be at least %s" pp_var x
               stransient)
         lvls);
  ...
  let do_constraint (s1, s2) =
    let get s =
      try Hashtbl.find tbl s
      with Not_found -> error ~loc:f.f_loc "unbound security level %s" s
    in
    try VlPairs.add_le (get s1) (get s2)
    with Lvl.Unsat _unsat ->
      error ~loc:f.f_loc "cannot add constraint %s <= %s, it is inconsistent" s1
        s2
  in
  ...
  let do_local x venv =
    ...
    Env.add_var env venv x vk vty
  in

  let venv = Sv.fold do_local (locals f) venv in

  let do_spill x venv =
    Option.map_default (fun sx -> do_local sx venv) venv (Env.add_spill env x)
  in
...
and ty_fun_infer is_ct_asm fenv fn =
  ...
  let doout x (omsf, ty) =
    let le_ty ty1 ty2 =
      try
        ...
        | Indirect (lp1, le1), Indirect (lp2, le2) ->
            VlPairs.add_le lp1 lp2;
            VlPairs.add_le le1 le2
        | _, _ -> assert false
      with Lvl.Unsat _unsat ->
        (* TODO: Specify info. *)
        error ~loc:(L.loc x)
          "return type for %a is %a it should be less than %a" pp_var_i x pp_vty
          ty1 pp_vty ty2
    in
    match omsf with
    | Some true ->
        MSF.check_msf_exact msf x;
        IsMsf
    | Some false ->
        if MSF.is_msf_exact msf x then
          error ~loc:(L.loc x) "return annotation for %a should be %s" pp_var_i
            x smsf;
        le_ty (Env.get_i ~public:false venv x) ty;
        IsNormal ty
    | None ->
  ...
  let add ls vty =
    ...
    | IsNormal (Indirect ((np, sp), (ne, se))) -> np :: sp :: ne :: se :: ls
  in
  ...
  if !Glob_options.debug then
    Format.eprintf "Before optimization:@.%a@." pp_funty (f.f_name.fn_name, fty);
...
let ty_prog is_ct_asm (prog : ('info, 'asm) prog) fl =
  ...
  let fl =
    if fl = [] then List.rev_map (fun f -> f.f_name) prog
    else
      let get fn =
        try (List.find (fun f -> f.f_name.fn_name = fn) prog).f_name
        with Not_found ->
          hierror ~loc:Lnone ~kind:"speculative constant type checker"
            "unknown function %s" fn
      in
      List.map get fl
  in
